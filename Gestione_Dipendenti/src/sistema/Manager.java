...
public class Manager extends Dipendenti {
	...
	public static void visualizzaManager(Connection conn, Scanner scanner) {
		...
		String query = "SELECT manager.id, manager.idDipendente, nome, cognome" + " FROM azienda.dipendenti" + " INNER JOIN azienda.manager"
	...
	public static void visualizzaManagerConID(Connection conn, Scanner scanner) {
		...
		String query = "SELECT dipendenti.idDipendente, nome, cognome, team.nomeTeam, progetti.nomeProgetto"
				+ " FROM azienda.dipendenti" + " INNER JOIN azienda.manager"
				+ " ON dipendenti.idDipendente = manager.idDipendente" + " LEFT JOIN azienda.team"
				+ " ON dipendenti.idDipendente = team.idTeamLeader" + " LEFT JOIN azienda.progetti"
				+ " ON manager.idDipendente = progetti.idProjectManager" + " WHERE manager.id = ? ;";
		...
		try (PreparedStatement pstmt = conn.prepareStatement(query)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					...
					int idDipendente = rs.getInt("idDipendente");
					System.out.printf(" idDipendente: %d | nome: %s | cognome: %s | team: %s | progetto: %s ",
							idDipendente, nome, cognome, nomeTeam, nomeProgetto);
				}
	...
	public static void modificaBonusManager(Connection conn, Scanner scanner) {
		String query = "UPDATE azienda.manager" + " SET bonus = ?" + " WHERE id = ?;";
	...
	public static void calcolaStipendioManager(Connection conn) {
		String query = "SELECT nome, cognome, stipendio, manager.bonus" + " FROM azienda.dipendenti"
				+ " INNER JOIN azienda.manager" + " ON dipendenti.idDipendente = manager.idDipendente;";
